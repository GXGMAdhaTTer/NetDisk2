...
void UserCallbackLib::userSignupPost(const wfrest::HttpReq* req, wfrest::HttpResp* resp, SeriesWork* series) {
    ...
    std::string url = "http://10.211.55.5:8500/v1/agent/services";

    auto httpTask = WFTaskFactory::create_http_task(url, 0, 0, [username, password, resp](WFHttpTask* httpTask) {  //最好是把resp放在context中
        // 解析consul的响应内容
        ...
        auto rpcTask = client.create_Signup_task([resp](RespSignup* response, srpc::RPCContext* context) {
            if (context->success() && response->code() == 0) {
                resp->String("SUCCESS");
            } else {
                fprintf(stderr, "status = %d, error = %d, errmsg = %s\n", context->get_status_code(), context->get_error(), context->get_errmsg());
                resp->String("FAIL");
            }
        });
